...
const PFI_MODEL_PATH = "/models/chariot-manuel-palan-manuel.glb";
const MESH_MAPPING_STORAGE_KEY = "oyste-3d-lab:manual-trolley-manual-hoist:mesh-mapping";
const DEV_PRESET_STORAGE_KEY = "oyste-3d-lab:manual-trolley-manual-hoist:developer-preset";
const EXTRACTION_PRESET_STORAGE_KEY =
  "oyste-3d-lab:manual-trolley-manual-hoist:mesh-022-extraction";
const MESH_VALIDATION_STORAGE_KEY = "oyste-3d-lab:manual-trolley-manual-hoist:mesh-validation";
const FREE_EXTRACTIONS_STORAGE_KEY = "oyste-3d-lab:manual-trolley-manual-hoist:free-extractions";
const FUNCTIONAL_GROUPS_STORAGE_KEY = "oyste-3d-lab:manual-trolley-manual-hoist:functional-groups";
const IGNORED_ISLANDS_STORAGE_KEY = "oyste-3d-lab:manual-trolley-manual-hoist:ignored-islands";
...
type SpecialObjectDescriptor = {
  key: string;
  name: string;
  type: "Points" | "Sprite" | "Line" | "LineSegments" | "LineLoop";
  parentName: string;
  hiddenAutomatically: boolean;
};
type MicroIslandDescriptor = {
  key: string;
  meshKey: string;
  triangles: number;
  width: number;
...
type ExtractionPreset = {
  version: 1;
  model: "PFI125_MANUAL_TROLLEY_MANUAL_HOIST";
  sourceMesh: "mesh-022";
...
const INITIAL_FUNCTIONAL_GROUPS: FunctionalGroup[] = [];
...
function removeMicroIslands(
  geometry: THREE.BufferGeometry,
  maxTriangles = 2,
  maxDimension = 2.5,
) {
  const islands = splitGeometryIntoIslands(geometry);
  const removed: Array<{
    triangles: number;
    width: number;
    height: number;
  ...
  islands.forEach((island) => {
    ...
    const isMicroIsland =
      island.triangles <= maxTriangles && largestDimension <= maxDimension;
    ...
    const islandIndex = island.geometry.getIndex();
    if (islandIndex) {
      for (let offset = 0; offset < islandIndex.count; offset += 1) {
        keptIndices.push(islandIndex.getX(offset));
      }
    }
    island.geometry.dispose();
  });

  if (!removed.length || !keptIndices.length) {
  ...
  return { geometry: cleaned, removed };
}
...
function PfiModel({
  ...
  onSpecialObjects: (objects: SpecialObjectDescriptor[]) => void;
  onMicroIslands: (islands: MicroIslandDescriptor[]) => void;
  onFocusPoint: (point: Vector3Tuple) => void;
  hiddenIslands: Set<string>;
  ignoredIslands: Set<string>;
  isolatedIsland: string | null;
  onIslands: (islands: IslandDescriptor[]) => void;
  islandIdentificationMode: boolean;
  onIslandIdentified: (islandKey: string) => void;
  ...
  const {
    ...
    sourceMesh022,
      ...
      const specialObjectDescriptors: SpecialObjectDescriptor[] = [];
      const microIslandDescriptors: MicroIslandDescriptor[] = [];
      let meshIndex = 0;
      let specialObjectIndex = 0;
      prepared.traverse((object) => {
        ...
        const isLineLoop = object instanceof THREE.LineLoop;
        const isLine =
          object instanceof THREE.Line && !isLineSegments && !isLineLoop;

        if (isPoints || isSprite || isLineSegments || isLineLoop || isLine) {
          specialObjectIndex += 1;
          const type: SpecialObjectDescriptor["type"] = isPoints
            ? "Points"
            : isSprite
              ? "Sprite"
              : isLineSegments
          ...
          object.userData.oysteAutoHiddenSpecialObject = true;
          specialObjectDescriptors.push({
            key: `special-${String(specialObjectIndex).padStart(3, "0")}`,
            name: object.name || `${type} ${specialObjectIndex}`,
            type,
            parentName: object.parent?.name || "Sans parent",
            hiddenAutomatically: true,
          });
          return;
        }
        ...
        // Preserve the complete source geometry. The full island inspector
        // below exposes every disconnected component so the operator can
        // confirm visually before ignoring a fragment.
      ...
      prepared.traverse((object) => {
        ...
        if (!source.isMesh || !source.parent) return;
        const meshKey = source.userData.oysteMeshKey as string | undefined;
        if (!meshKey) return;
        if (meshKey === "mesh-022") mesh022 = source;
        ...
        islandGroup.name = `${meshKey}-islands`;
        ...
        const keptIndices: number[] = [];
        const renderedFaceToIslandKey: string[] = [];
        const detectedIslands = splitGeometryIntoIslands(original);

        detectedIslands.forEach((island, index) => {
          const islandKey = `${meshKey}-island-${String(index + 1).padStart(3, "0")}`;
          const size = new THREE.Vector3();
          island.geometry.boundingBox?.getSize(size);
          const islandIndex = island.geometry.getIndex();
          ...
          if (
            !ignoredIslands.has(islandKey) &&
            !hiddenIslands.has(islandKey) &&
            islandIndex
          ) {
            for (let offset = 0; offset < islandIndex.count; offset += 1) {
              keptIndices.push(islandIndex.getX(offset));
            ...
            for (
              let triangle = 0;
              triangle < islandIndex.count / 3;
              triangle += 1
            ) {
              renderedFaceToIslandKey.push(islandKey);
            }
          }
          ...
          islandMesh.userData.oysteIslandKey = islandKey;
          islandMesh.userData.oysteParentMeshKey = meshKey;
        ...
        const filteredGeometry = original.clone();
        filteredGeometry.setIndex(keptIndices);
        if (keptIndices.length) filteredGeometry.computeVertexNormals();
        filteredGeometry.computeBoundingBox();
        filteredGeometry.computeBoundingSphere();
        source.geometry.dispose();
        source.geometry = filteredGeometry;
        source.userData.oysteRenderedFaceToIslandKey =
          renderedFaceToIslandKey;
      ...
      return {
        ...
        meshes: descriptors,
        specialObjects: specialObjectDescriptors,
        microIslands: microIslandDescriptors,
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (
        preset.version !== 1 ||
        preset.model !== "PFI125_MANUAL_TROLLEY_MANUAL_HOIST" ||
  ...
  useEffect(() => {
    model.traverse((object) => {
      ...
      if (islandKey) {
        ...
        mesh.visible =
          isolatedIsland === islandKey && !ignoredIslands.has(islandKey);
        return;
      }
  ...
  useEffect(
    () => onSpecialObjects(specialObjects),
    [onSpecialObjects, specialObjects],
  );
  useEffect(
    () => onMicroIslands(microIslands),
    [microIslands, onMicroIslands],
  );
  useEffect(() => onIslands(islands), [islands, onIslands]);

  useEffect(() => {
    if (!surfaceSelectionMode && !islandIdentificationMode) return;
    ...
    const handlePointerDown = (event: PointerEvent) => {
      ...
      if (islandIdentificationMode) {
        const faceIndex = hit.intersection.faceIndex;
        const faceToIslandKey = hit.candidate.source.userData
          .oysteRenderedFaceToIslandKey as string[] | undefined;
        const islandKey =
          faceIndex != null ? faceToIslandKey?.[faceIndex] : undefined;
        if (islandKey && !ignoredIslands.has(islandKey)) {
          onIslandIdentified(islandKey);
        }
        return;
      }
...
export default function OysteManualTrolleyManualHoist3DLab() {
  ...
  const [specialObjects, setSpecialObjects] = useState<
    SpecialObjectDescriptor[]
  >([]);
  const [microIslands, setMicroIslands] = useState<MicroIslandDescriptor[]>([]);
  ...
  const [ignoredIslands, setIgnoredIslands] = useState<Set<string>>(
    () => new Set(),
  );
  const [isolatedIsland, setIsolatedIsland] = useState<string | null>(null);
  const [islandIdentificationMode, setIslandIdentificationMode] =
    useState(false);
  const [identifiedIslandKey, setIdentifiedIslandKey] = useState<string | null>(
    null,
  );
  ...
  const identifiedIsland = useMemo(
    () =>
      identifiedIslandKey
        ? islands.find((island) => island.key === identifiedIslandKey) ?? null
        : null,
    [identifiedIslandKey, islands],
  );

  useEffect(() => {
    if (!identifiedIslandKey) return;
    setOpenSections((current) => ({ ...current, decomposition: true }));
    requestAnimationFrame(() => {
      document
        .getElementById(`island-card-${identifiedIslandKey}`)
  ...
  useEffect(() => {
    try {
      ...
      const savedIgnoredIslands = localStorage.getItem(
        IGNORED_ISLANDS_STORAGE_KEY,
      );
      if (savedIgnoredIslands) {
        setIgnoredIslands(new Set(JSON.parse(savedIgnoredIslands) as string[]));
      }
      const savedFunctionalGroups = localStorage.getItem(
        FUNCTIONAL_GROUPS_STORAGE_KEY,
      );
      const parsedFunctionalGroups: FunctionalGroup[] = savedFunctionalGroups
        ? JSON.parse(savedFunctionalGroups)
        : [];
      if (parsedFunctionalGroups.length) {
        setFunctionalGroups(parsedFunctionalGroups);
      }
      ...
      const parsedValidation: MeshValidationMap = savedValidation
        ? JSON.parse(savedValidation)
        : {};

      // This GLB stores visible chain/hook geometry directly on mesh-001.
      // Keep the root node protected so its children remain mounted, but attach
      // its own drawable geometry to the Palan group. The visibility engine
      // already hides root triangles through material + drawRange without ever
      // disabling the hierarchy itself.
      const palanGroup = parsedFunctionalGroups.find(
        (group) =>
          group.id === "palan" ||
          group.label.trim().toLowerCase() === "palan",
      );
      const migratedValidation = palanGroup
        ? {
            ...parsedValidation,
            [ROOT_MESH_KEY]: {
              status: "validated" as MeshValidationStatus,
              group: palanGroup.id,
              note: "Géométrie racine : chaîne et crochet du palan",
              updatedAt:
                parsedValidation[ROOT_MESH_KEY]?.updatedAt ??
                new Date().toISOString(),
            },
          }
        : parsedValidation;

      setMeshValidation(migratedValidation);
      localStorage.setItem(
        MESH_VALIDATION_STORAGE_KEY,
        JSON.stringify(migratedValidation),
      );
    } catch {
  ...
  }, [freeExtractions, functionalGroups, meshValidation]);

  const createFunctionalGroup = useCallback(() => {
    ...
    setFunctionalGroups((current) => {
      let id = baseId;
      let suffix = 2;
      while (current.some((group) => group.id === id)) {
        id = `${baseId}-${suffix}`;
        suffix += 1;
      }
      const next = [
        ...current,
        {
          id,
          label,
          description: newGroupDescription.trim() || "Groupe métier personnalisé",
          meshKeys: [],
        },
      ];
      localStorage.setItem(FUNCTIONAL_GROUPS_STORAGE_KEY, JSON.stringify(next));
      if (!freeExtractionGroup) setFreeExtractionGroup(id);
      if (!freeExtractionRemainingGroup) setFreeExtractionRemainingGroup(id);
      return next;
    });
    setNewGroupName("");
  ...
  const deleteFunctionalGroup = useCallback((groupId: FunctionalGroupId) => {
    setFunctionalGroups((current) => {
    ...
    setMeshValidation((current) => {
      const next = { ...current };
      Object.entries(next).forEach(([meshKey, record]) => {
    ...
    setIsolatedGroup((current) => (current === groupId ? null : current));
    setFreeExtractionGroup((current) => (current === groupId ? "" : current));
    setFreeExtractionRemainingGroup((current) =>
  ...
  const ignoreIsland = useCallback((key: string) => {
    setIgnoredIslands((current) => {
      const next = new Set(current);
      next.add(key);
      localStorage.setItem(
        IGNORED_ISLANDS_STORAGE_KEY,
    ...
    setIsolatedIsland((current) => (current === key ? null : current));
  }, []);

  const restoreIsland = useCallback((key: string) => {
    setIgnoredIslands((current) => {
      const next = new Set(current);
      next.delete(key);
      localStorage.setItem(
          ...
          <div className="grid xl:grid-cols-[minmax(0,1fr)_520px]">
            <div className="relative min-h-[860px] bg-slate-950">
              <div className="pointer-events-none absolute inset-x-0 top-0 z-10 flex items-center justify-between gap-3 p-5">
                <div className="rounded-full border border-white/10 bg-white/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.28em] text-white/80">
                  OYSTE 3D Lab · Chariot manuel + palan manuel
                </div>
                <div className="rounded-full border border-emerald-300/30 bg-emerald-400/10 px-4 py-2 text-xs font-semibold text-emerald-100">
                  {stats.meshes
                    ? `${stats.meshes} meshes · ${namedCount} identifiés${
                        specialObjects.length
                          ? ` · ${specialObjects.length} objet${
                              specialObjects.length > 1 ? "s" : ""
                            } spécial${
                              specialObjects.length > 1 ? "aux" : ""
                            } masqué${
                              specialObjects.length > 1 ? "s" : ""
                            }`
                          : ""
                        ...
                        microIslands.length
                          ? ` · ${microIslands.length} micro-îlot${
                              microIslands.length > 1 ? "s" : ""
                            } nettoyé${
                              microIslands.length > 1 ? "s" : ""
                            }`
                          : ""
              ...
              >
                ...
                <Suspense fallback={<LoadingModel />}>
                  <PfiModel
                    ...
                    onSpecialObjects={setSpecialObjects}
                    onMicroIslands={setMicroIslands}
                    onFocusPoint={(point) => {
                      setFocusPoint(point);
                      setFocusKey((value) => value + 1);
                    }}
                    hiddenIslands={hiddenIslands}
                    ignoredIslands={ignoredIslands}
                    isolatedIsland={isolatedIsland}
                    onIslands={setIslands}
                    islandIdentificationMode={islandIdentificationMode}
                    onIslandIdentified={(islandKey) => {
                      setIdentifiedIslandKey(islandKey);
                      setIsolatedIsland(null);
                      setIsolatedMesh(null);
                      setIsolatedGroup(null);
                    }}
            ...
            <aside className="flex max-h-[980px] flex-col bg-white">
              ...
              <div className="min-h-0 flex-1 overflow-y-auto p-4 space-y-3">
                  ...
                  <div className="space-y-3">
                    <div className="rounded-2xl border border-sky-200 bg-sky-50 p-3">
                      <p className="text-xs font-black uppercase tracking-[0.14em] text-sky-800">
                        Cartographie spécifique à cette variante
                      </p>
                      <p className="mt-1 text-[11px] leading-relaxed text-sky-950">
                        Aucun groupe n’est imposé. Crée les groupes métier utiles,
                        puis affecte progressivement les meshes du modèle.
                      </p>
                      <div className="mt-3 space-y-2">
                        <input
                          ...
                          onKeyDown={(event) => {
                            if (event.key === "Enter") createFunctionalGroup();
                          }}
                          placeholder="Nom du groupe, ex. Chariot manuel"
                          className="w-full rounded-xl border border-sky-200 bg-white px-3 py-2 text-xs font-bold text-slate-900"
                        />
                        <input
                          ...
                          className="w-full rounded-xl border border-sky-200 bg-white px-3 py-2 text-xs text-slate-700"
                        />
                        <button
                          type="button"
                          disabled={!newGroupName.trim()}
                          onClick={createFunctionalGroup}
                          className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-sky-700 px-3 py-2.5 text-xs font-black text-white disabled:cursor-not-allowed disabled:opacity-40"
                        >
                          <Layers3 className="h-4 w-4" /> Créer le groupe
                        </button>
                      </div>
                    </div>
                    ...
                    {effectiveFunctionalGroups.map((group) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <button
                            type="button"
                            onClick={() => deleteFunctionalGroup(group.id)}
                            className="mt-2 w-full rounded-xl border border-rose-200 bg-white py-2 text-[11px] font-bold text-rose-700"
                          >
                            Supprimer ce groupe
                          </button>
                        </div>
                  ...
                  <div className="mt-4 rounded-2xl border border-sky-200 bg-sky-50 p-3">
                    ...
                    <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
                      ...
                      <select
                        ...
                        disabled={functionalGroups.length === 0}
                        ...
                        {freeExtractions.map((item) => (
                          ...
                          >
                            ...
                            <span className="text-sky-700">
                              ...
                              {item.remainingGroup
                                ? ` · reste → ${functionalGroups.find((group) => group.id === item.remainingGroup)?.label ?? item.remainingGroup}`
                                : ""}
                            </span>
                          </div>
                        ))}
                ...
                <Section
                  title="Inspecteur géométrique — tous les îlots"
                ...
                >
                  <div className="mb-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
                    <b>{islands.length || "—"} îlots géométriques détectés sur l’ensemble du modèle.</b>{" "}
                    Active l’identification, puis clique directement sur un point
                    blanc. Le clic utilise désormais exactement le même moteur que
                    la sélection de surface, puis traduit le triangle touché vers
                    son îlot.
                  </div>
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      setIslandIdentificationMode((current) => !current);
                      setSurfaceSelectionMode(false);
                      setIsolatedIsland(null);
                    }}
                    className={`mb-3 inline-flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3 text-xs font-black ${
                      islandIdentificationMode
                  ...
                  >
                    <MousePointer2 className="h-4 w-4" />
                    {islandIdentificationMode
                      ? "Mode identification actif — clique sur le point"
                      : "Identifier un îlot par clic"}
                  </button>

                  {identifiedIsland ? (
                    <div className="mb-3 rounded-2xl border-2 border-fuchsia-300 bg-fuchsia-50 p-3">
                      <p className="text-[10px] font-black uppercase tracking-[0.13em] text-fuchsia-700">
                        Îlot identifié
                      </p>
                      <p className="mt-1 text-sm font-black text-slate-950">
                        {identifiedIsland.key}
                      </p>
                      <p className="mt-1 text-[11px] text-slate-600">
                        Source : {identifiedIsland.meshKey} ·{" "}
                        {identifiedIsland.meshName}
                      </p>
                      <p className="text-[11px] text-slate-600">
                        {identifiedIsland.triangles} triangles ·{" "}
                        {round(identifiedIsland.width, 3)} ×{" "}
                        {round(identifiedIsland.height, 3)} ×{" "}
                        {round(identifiedIsland.depth, 3)}
                      </p>
                      <div className="mt-3 grid grid-cols-2 gap-2">
                        <button
                          type="button"
                          onClick={() => {
                            setIslandIdentificationMode(false);
                            setIsolatedMesh(null);
                            setIsolatedGroup(null);
                            setIsolatedIsland(identifiedIsland.key);
                          }}
                          className="rounded-xl bg-slate-950 py-2.5 text-xs font-black text-white"
                        >
                          <Crosshair className="mr-1 inline h-4 w-4" />
                          Isoler
                        </button>
                        <button
                          type="button"
                          onClick={() => {
                            ignoreIsland(identifiedIsland.key);
                            setIdentifiedIslandKey(null);
                          }}
                          className="rounded-xl border border-rose-200 bg-white py-2.5 text-xs font-black text-rose-700"
                        >
                          Ignorer
                        </button>
                      </div>
                  ...
                  <div className="max-h-[420px] space-y-2 overflow-y-auto pr-1">
                    {islands.map((island) => {
                      ...
                      const ignored = ignoredIslands.has(island.key);
                      const isolated = isolatedIsland === island.key;
                      return (
                        <div
                          id={`island-card-${island.key}`}
                          key={island.key}
                          className={`rounded-xl border p-3 ${
                            identifiedIslandKey === island.key
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            <div>
                              ...
                              <p className="text-[10px] font-bold text-sky-700">
                                Source : {island.meshKey} · {island.meshName}
                              </p>
                              <p className="text-[10px] text-slate-500">
                                {island.triangles} triangles ·{" "}
                                {round(island.width, 3)} ×{" "}
                                {round(island.height, 3)} ×{" "}
                                {round(island.depth, 3)}
                              </p>
                            </div>
                            {ignored ? (
                              <span className="rounded-full bg-rose-100 px-2 py-1 text-[9px] font-black uppercase text-rose-700">
                                Ignoré
                              </span>
                            ) : null}
                          </div>
                          <div className="mt-2 grid grid-cols-3 gap-2">
                            <button
                              ...
                              className="rounded-lg border border-slate-200 bg-white py-2 text-[11px] font-bold disabled:opacity-40"
                            >
                              {hidden ? (
                                <Eye className="mr-1 inline h-4 w-4" />
                              ) : (
                            ...
                            <button
                              ...
                              className={`rounded-lg py-2 text-[11px] font-bold text-white disabled:opacity-40 ${
                                isolated ? "bg-violet-700" : "bg-slate-950"
                              }`}
                            >
                              <Crosshair className="mr-1 inline h-4 w-4" />
                            ...
                            <button
                              ...
                              onClick={() =>
                                ignored
                                  ? restoreIsland(island.key)
                                  : ignoreIsland(island.key)
                              }
                  ...
                  <button
                    ...
                    onClick={() => {
                      setHiddenIslands(new Set());
                      setIgnoredIslands(new Set());
                      localStorage.removeItem(IGNORED_ISLANDS_STORAGE_KEY);
                      setIslandIdentificationMode(false);
                      setIdentifiedIslandKey(null);
                      setIsolatedIsland(null);
                    }}
                  ...
                  >
                    Restaurer et réafficher tous les îlots
                  </button>
                </Section>
                ...
                {microIslands.length ? (
                  <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-3">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <p className="text-xs font-black uppercase tracking-[0.12em] text-emerald-900">
                          Micro-îlots nettoyés
                        </p>
                        <p className="mt-1 text-xs leading-5 text-emerald-800">
                          Les fragments de 1 à 2 triangles mesurant moins de
                          2,5 unités ont été retirés automatiquement.
                        </p>
                      </div>
                    </div>
                    <div className="mt-3 space-y-1.5">
                      {microIslands.map((island) => (
                        <div
                          key={island.key}
                        ...
                        >
                          <div>
                            ...
                            <p className="text-slate-500">
                              {island.triangles} triangle
                              {island.triangles > 1 ? "s" : ""} ·{" "}
                              {round(island.width, 3)} ×{" "}
                              {round(island.height, 3)} ×{" "}
                              {round(island.depth, 3)}
                            </p>
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <p className="text-xs font-black uppercase tracking-[0.12em] text-amber-900">
                          Objets non-Mesh détectés
                        </p>
                        <p className="mt-1 text-xs leading-5 text-amber-800">
                          Ces objets techniques sont masqués automatiquement,
                          car ils ne font pas partie des composants configurables.
                        </p>
                      </div>
                    </div>
                    <div className="mt-3 space-y-1.5">
                      {specialObjects.map((object) => (
                        <div
                          key={object.key}
                          className="flex items-center justify-between rounded-xl border border-amber-100 bg-white px-3 py-2 text-xs"
                        >
                          <div>
                            <p className="font-bold text-slate-900">
                              {object.name}
                            ...
                            <p className="text-slate-500">
                              {object.type} · parent : {object.parentName}
                            </p>
                          </div>
                          <span className="rounded-full bg-emerald-100 px-2 py-1 font-bold text-emerald-700">
                            Masqué
                      ...
                      >
                        ...
                        {isRoot ? (
                          <div className="mt-2 rounded-xl border border-emerald-200 bg-white/70 px-3 py-2 text-center text-xs font-bold text-emerald-700">
                            Racine protégée · sa géométrie visible est pilotée par le groupe Palan
                          </div>
                        ) : (
                          <>
              ...
              <div className="border-t border-slate-200 bg-slate-50 p-4">
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 text-center text-xs sm:grid-cols-4">
                  ...
                  <div className="rounded-xl bg-white p-2">
                    <CircleAlert className="mx-auto h-4 w-4 text-amber-600" />
                    <b>{specialObjects.length + microIslands.length}</b>
                    <p className="text-slate-400">Parasites nettoyés</p>
                  </div>
                </div>
              </div>
            </aside>
