...
export default function AuthForm({ mode }: { mode: Mode }) {
  ...
  const inputClass = "mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3.5 text-sm outline-none transition focus:border-[#007f8f] focus:ring-4 focus:ring-cyan-50";
  return (
    <form onSubmit={submit} className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-xl shadow-slate-950/5 sm:p-8">
      ...
      {mode === "register" && <div className="mt-6 grid gap-4 sm:grid-cols-2">
        <label className="text-xs font-black uppercase tracking-wide text-slate-600 sm:col-span-2">Société *<input name="company" required autoComplete="organization" className={inputClass} /></label>
      ...
      <div className={`${mode === "register" ? "mt-4" : "mt-6"} grid gap-4`}>
        <label className="text-xs font-black uppercase tracking-wide text-slate-600">Adresse e-mail *<input name="email" required type="email" autoComplete="email" className={inputClass} /></label>
