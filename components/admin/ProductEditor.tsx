...
import { ArrowLeft, Check, FileText, GripVertical, ImagePlus, Link2, Plus, Save, Search, Trash2, UploadCloud } from "lucide-react";
  ...
  return <main className="mx-auto w-full max-w-[1500px] p-4 md:p-7 xl:p-9">
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
      <section className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
        ...
        {active === "Fiches techniques" && <div><h2 className="text-xl font-black">Documents téléchargeables</h2><p className="mt-1 text-sm text-slate-500">Fiches techniques, notices, certificats et plans d’encombrement.</p><button className="mt-6 inline-flex items-center gap-2 rounded-xl bg-slate-950 px-4 py-3 text-xs font-black text-white"><FileText size={17} /> Ajouter un document</button><div className="mt-5 space-y-3">{["Fiche technique", "Notice d’installation", "Plan d’encombrement"].slice(0, Math.max(1, product.documentCount)).map((name, index) => <article key={name} className="flex items-center gap-4 rounded-2xl border border-slate-200 p-4"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-red-50 text-red-600"><FileText size={20} /></div><div className="min-w-0 flex-1"><input defaultValue={`${name} ${product.code || "nouveau produit"}`} className="w-full truncate bg-transparent text-sm font-black outline-none" /><p className="mt-1 text-[10px] font-bold text-slate-400">PDF · Visible sur la fiche produit</p></div><button className="text-slate-400"><Trash2 size={17} /></button></article>)}</div></div>}
        {active === "Caractéristiques" && <div><h2 className="text-xl font-black">Caractéristiques techniques</h2><p className="mt-1 text-sm text-slate-500">Ces informations alimentent le tableau technique de la fiche produit.</p><div className="mt-6 space-y-3">{(product.features?.length ? product.features : [{ label: "Capacité", value: "" }, { label: "Dimensions", value: "" }]).map((feature, index) => <div key={`${feature.label}-${index}`} className="grid gap-3 rounded-2xl border border-slate-200 p-3 sm:grid-cols-[1fr_1.5fr_auto]"><input defaultValue={feature.label} placeholder="Libellé" className={field} /><input defaultValue={feature.value} placeholder="Valeur" className={field} /><button className="flex h-11 w-11 items-center justify-center rounded-xl text-slate-400 hover:bg-red-50 hover:text-red-600"><Trash2 size={17} /></button></div>)}</div><button className="mt-4 inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-black"><Plus size={16} /> Ajouter une caractéristique</button></div>}
        {active === "SEO" && <div><h2 className="text-xl font-black">Référencement naturel</h2><p className="mt-1 text-sm text-slate-500">Contrôlez l’apparence de la page dans Google.</p><div className="mt-6 space-y-5"><label className="block text-xs font-black text-slate-600">URL de la page<div className="mt-2 flex rounded-xl border border-slate-200"><span className="flex items-center border-r border-slate-200 bg-slate-50 px-3 text-xs text-slate-400">/catalogue/…/</span><input defaultValue={product.slug} className="min-w-0 flex-1 rounded-r-xl px-3 py-3 text-sm outline-none" /></div></label><label className="block text-xs font-black text-slate-600">Titre SEO<input defaultValue={`${product.name} | OYSTE`} className={`${field} mt-2`} /><span className="mt-1 block text-right text-[10px] text-slate-400">{product.name.length + 8} / 60</span></label><label className="block text-xs font-black text-slate-600">Meta description<textarea defaultValue={product.description} rows={4} className={`${field} mt-2`} /></label><div className="rounded-2xl border border-slate-200 p-5"><p className="text-xs font-black text-slate-400">Aperçu Google</p><p className="mt-4 text-lg text-blue-700">{product.name || "Nom du nouveau produit"} | OYSTE</p><p className="mt-1 text-xs text-emerald-700">oyste.fr › catalogue › {product.slug || "nouveau-produit"}</p><p className="mt-2 text-sm text-slate-600">{product.description || "La description SEO de votre produit apparaîtra ici."}</p></div></div></div>}
      </section>

      <aside className="space-y-5"><section className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm"><h3 className="text-sm font-black">État de la fiche</h3><div className="mt-5 space-y-4"><div><div className="flex justify-between text-xs font-black"><span>Complétude</span><span>{product.completeness || 20}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-[#007f8f]" style={{ width: `${product.completeness || 20}%` }} /></div></div><div className="space-y-2 text-xs font-bold text-slate-500">{["Informations générales", "Prix et disponibilité", "Description", "Image principale", "Référencement"].map((item, index) => <p key={item} className="flex items-center gap-2"><span className={`flex h-5 w-5 items-center justify-center rounded-full ${index < 3 ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-400"}`}>{index < 3 ? <Check size={12} /> : "·"}</span>{item}</p>)}</div></div></section><section className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm"><h3 className="text-sm font-black">Visibilité</h3><div className="mt-4 space-y-3"><label className="flex items-center justify-between text-xs font-bold text-slate-600"><span>Visible sur le site</span><input type="checkbox" defaultChecked={status === "Publié"} className="h-4 w-4 accent-[#007f8f]" /></label><label className="flex items-center justify-between text-xs font-bold text-slate-600"><span>Disponible à la vente</span><input type="checkbox" defaultChecked className="h-4 w-4 accent-[#007f8f]" /></label><label className="flex items-center justify-between text-xs font-bold text-slate-600"><span>Mis en avant</span><input type="checkbox" className="h-4 w-4 accent-[#007f8f]" /></label></div></section><section className="rounded-[1.5rem] bg-[#07131f] p-5 text-white shadow-sm"><Link2 size={20} className="text-cyan-300" /><h3 className="mt-4 text-sm font-black">Aperçu boutique</h3><p className="mt-2 text-xs leading-5 text-white/55">Contrôlez le rendu public avant de publier vos modifications.</p><button className="mt-4 w-full rounded-xl bg-white px-4 py-3 text-xs font-black text-slate-950">Voir la fiche produit</button></section></aside>
    </div>
  </main>;
}
